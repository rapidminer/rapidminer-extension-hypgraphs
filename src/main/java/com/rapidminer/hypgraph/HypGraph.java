package com.rapidminer.hypgraph;
...
public class HypGraph extends Operator {
	...
	public HypGraph(OperatorDescription description) {
		super(description);
	...
	public void doWork() throws OperatorException {
		...
		if (numHypoCols != numHypoRows || numDataCols != numDataRows || numHypoCols != numDataCols) {
			throw new UserError(this, "hypgraph.nonsymmetric");
		}
		...
		while (dataReader.hasNext()) {
			...
			for (Attribute att : ex.getAttributes()) {
				...
				if (dataValue >= 0) {
					...
					throw new UserError(this, "hypgraph.negativeValues", "data");
				}
				j++;
			}
			i++;
		...
		while (hypoReader.hasNext()) {
			...
			for (Attribute att : ex.getAttributes()) {
				...
				if (dataValue >= 0) {
					...
					throw new UserError(this, "hypgraph.negativeValues", "hypothesis");
				}
				j++;
			}
			i++;
